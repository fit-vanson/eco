...
function onPaginationDataLoad(page) {
	...
	$.ajax({
		url:base_url + "/backend/getMultipleSitesTableData?page="+page
...
function onRefreshData() {
	...
	$.ajax({
		url:base_url + "/backend/getMultipleSitesTableData?status="+$("#view_by_status").val()
		+"&search="+$("#search").val()
		// +"&language_code="+$('#language_code').val()
		+"&category_id="+$('#category_id').val(),
		// +"&brand_id="+$('#brand_id').val()
...
function onSearch() {
	...
	$.ajax({
		url: base_url + "/backend/getMultipleSitesTableData?search="+$("#search").val()
		+"&status="+$("#view_by_status").val()
		// +"&language_code="+$('#language_code').val()
		+"&category_id="+$('#category_id').val(),
		// +"&brand_id="+$('#brand_id').val()
...
function onConfirmWhenAddEdit() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/saveMultipleSitesData',
		...
		success: function (response) {
			...
			if (msgType == "success") {
				...
				onSuccessMsg(msg);
				var id = response.id;
				window.location.href= base_url + '/backend/site/'+id;
			} else {
				onErrorMsg(msg);
			}
